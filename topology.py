...
class Topology:
    ...
    def print_topology_shortest(self, shortest):
        for i in range(0, len(shortest["a_router"])):
            print(f"{shortest['a_router'][i]} -> {shortest['b_router'][i]} = {shortest['length'][i]}\t{shortest['through'][i]}")
...
#        self.print_shortest_path_trees()
    ...
    def create_shortest_link(self, router):
        ...
        next_router, length, through = self.find_shortest_not_visited(self.shortest_path_trees["trees"][router], visited, router)
        while next_router != -1:
            visited[next_router - 1] = True
            self.check_shortest_for_router_links(self.shortest_path_trees["trees"][router], router, next_router, length, through)
            next_router, length, through = self.find_shortest_not_visited(self.shortest_path_trees["trees"][router], visited, router)
